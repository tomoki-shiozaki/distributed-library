...
| 画面ID | 画面名       | 概要                       | 備考               |
|--------|--------------|----------------------------|------------------|
| L001   | ホーム画面 | トップページ。利用する機能（アカウント機能・本の登録・本の利用）を選ぶ。 | 状態（ログアウト・司書ログイン・一般ユーザーログイン）で利用できる機能が変わる | 
| L002   | 本の登録画面 | 司書が本のデータを入力し、新しい本を登録する         | ISBNを入力すると、APIで本データを取得できる |
| L003   | 本の検索画面   | ユーザーがタイトル、著者名で本を検索する   | タイトル、著者名はAND, OR検索を選べるようにする |
...
| L008   | ユーザーが貸出・予約中の本一覧画面 | 自分の本の貸出・予約の状況を確認する | |
...
| 画面ID | 画面名       | 概要                       |  備考               |
|--------|--------------|----------------------------|---------------------|
| A001   | ホーム画面 | トップページ。ヘッダー右上のボタンからアカウント機能を操作する | ログイン前・ログイン後でアカウント機能が変わる | 
...
- ユーザーが予約すると「予約中」になります。

- 実際に本を受け取ると「貸出中」に更新され、貸出日・返却予定日が設定されます。

- 返却されると「返却済み」となり、貸出レコードが完結します。